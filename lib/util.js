...
var decodeRLP = exports.decodeRLP = function(obj) {
  ...
  if (Buffer.isBuffer(data)) {
    ...
    switch (true) {
      ...
      case (character <= 0xf7):
        ...
        for (var i = 0; i < len;) {
          ...
          // buff = Buffer.concat([buff, new Buffer([tmp.data.toJSON()])]);
        ...
        // TODO: fix this after figuring out 4th case
      case (character <= 0xff):
        console.log("5th case");
        var l = character - 0xf7
        var lenOfLen = Buffer.byteLength(data.slice(pos + 1, pos + 1 + l));
        pos = pos + len + 1
        prevPos = lenOfLen
        for (var i = 0; i < lenOfLen;) {
          ...
          buff = Buffer.concat([buff, tmp.data]);
          prevPos = tmp.pos;
          i += (prevPos - pos);
          pos = prevPos;
        }
    ...
    logger.error('[decodeRLP]: only accepting buffer objects');
  }
};
...
Array.prototype.formatBufferArray = function() {
  var retArray = [];
  if (Object.prototype.toString.call(this) === '[object Array]') {
    for (var i in this) {
      if (Object.prototype.toString.call(this[i]) === '[object Array]') {
        retArray.push(this[i].formatBufferArray);
      } else if (Buffer.isBuffer(this[i])) {
        retArray.push(this[i].toString());
      } else {
