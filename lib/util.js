 ...
 *
 * @param {Buffer|String} buffer
 * @returns {Buffer} - returns the Buffer object for sha3  result
 **/
var sha3Bin = exports.sha3Bin = function (buffer) {
  return new Buffer(sha3(buffer), 'hex');
}
...
/**
 * Constant: 256 bit filled with 0s
 * @return {Buffer} 256 bits of zero hash
 */
...
var currencyToString = exports.currencyToString = function (num) {
  if (Buffer.isBuffer(num)) {
    return currencyToString(bignum.fromBuffer(num));
  } else {
    if (num.cmp(Fee.units.ether.value) > 0) {
      return num.div(Fee.units.ether.value).toString() + " " + Fee.units.ether.unit;
    } else if (num.cmp(Fee.units.finney.value) > 0) {
      return num.div(Fee.units.finney.value).toString() + " " + Fee.units.finney.unit;
    } else if (num.cmp(Fee.units.szabo.value) > 0) {
      return num.div(Fee.units.szabo.value).toString() + " " + Fee.units.szabo.unit;
    } else if (num.cmp(Fee.units.wei.value) > 0) {
      return num.div(Fee.units.wei.value).toString() + " " + Fee.units.wei.unit;
    }
  }
};
