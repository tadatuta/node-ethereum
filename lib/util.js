  ...
  if (data) {
    ...
    if (typeof data == 'string' || data instanceof String) {
      if (dataBufferLen === 1 && dataBuffer[0] <= 0x7f) {
        ...
        buffer = new Buffer(0);
        var lenOfLen = Buffer.byteLength(dataBufferLen.toString(2));
        console.log(lenOfLen);

        return buffer.concat(new Buffer((0xb7+lenOfLen).toString(16), 'hex'), new Buffer(dataBufferLen), dataBuffer);
      }
      //if data is an array
