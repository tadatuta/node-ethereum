...
State.prototype.createContract = function (address, amount, root) {
  var contract = new State(address, amount, 0);
...
State.prototype.createStateFromBuffer = function (buf) {
 ...
 */
State.prototype.rlpEncode = function () {
  var stateRoot = new Buffer('');
  if (this.worldState) {
    stateRoot = this.worldState.trie.root;
  }
  return rlp.encode([this.amount, this.nonce, stateRoot, this.script]);
}
/**
