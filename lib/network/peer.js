var util = require('util'),
  ...
  Block = require('../schema/block.js'),
  Transaction = require('../schema/transaction.js'),
  internals = {};
...
internals.Peer.prototype._parseData = function (data) {
  ...
  var payloadLen = parseInt(data.slice(4, 8)
    .toString('hex'), 16),
    payload = RLP.decode(data.slice(8, payloadLen + 8));

  assert.equal(this.network.SYNC_TOKEN, data.slice(0, 4)
