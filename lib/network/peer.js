...
internals.Peer.prototype._onData = function(data) {
    ...
    this.network.emit("message", peer, payload, data);
    ...
    switch (payload[0][0]) {
        ...
        case 0x12:
            //transactions
            var message = internals.parseTxs(payload.slice(1));
            this.network.emit("message.transactions", this, message, payload, data);
            this.emit("message.transactions", message, payload, data);
            break;
...
internals.parseHeaders = function(rawHeader) {
    return {
        parentHash: rawHeader[0],
        sha3UncleList: rawHeader[1],
        coinbase: rawHeader[2],
        stateRoot: rawHeader[3],
        difficulty: rawHeader[4],
        timestamp: rawHeader[6],
        extraData: rawHeader[7],
        nonce: rawHeader[8]
    }
};

//parses an array of transactions
internals.parseTxs = function(rawTxs) {
    var txs = [];
